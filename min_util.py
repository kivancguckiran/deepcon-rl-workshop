...
class bandit:
    def __init__(self, eps=0, step_size=0., initial=0):
        self.epsilon = eps
    ...
    def step(self):
        ...
        if self.step_size == 0:
            self.q_estimates[idx] += (1.0 / self.time_step) * (reward - self.q_estimates[idx])
        else:
    ...
    def plot(self):
        plt.violinplot(positions=np.arange(self.k_arm), dataset=self.q_true + np.random.randn(100,self.k_arm))
